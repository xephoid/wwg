...
exports.generate = async (req, res) => {
  ...
  const initial = `The user has chosen their character name to be ${body.name}. They are a ${body.race}. Their ability scores are ${body.abilityScores[0].ability} +3 ${body.abilityScores[1].ability} +2 and ${body.abilityScores[2].ability} +1. Their ship's name is ${body.ship.name}. They are ready to begin their adventure. Please remember to only respond with JSON and nothing else.`;
  ...
  const generateResponse = async (prompt) => {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "developer",
          content: "You are a helpful assistant that generates game content. Always respond with valid JSON and nothing else."
        },
        {
          role: "user",
          content: initial
  ...
  switch (body.gameState) {
    ...
    case 'HABITAT_INTRO':
    default:
      json = await generateResponse(`Generate an exciting intro as their character arrives at ${body.habitatName} which is ${body.habitatDescription}. The result should be in JSON with the following format: { "title": "Fun name of the adventure based on the player's stats", "description": "An exciting intro"} with nothing else.`);
      break;
  }
